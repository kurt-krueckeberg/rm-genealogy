...
**Predicate:** `PersonTable` has primary key `PersonID` that indicates a unique individual in the tree, with sex `Sex`,
with unique `UniqueID` (apparently a `GUID` or hash value that is a standard **GEDCOM** field), parent or parents
`ParentID` and spouse `SpouseID`, which is almost always zero (when it isn't, it functions like a foreign key
