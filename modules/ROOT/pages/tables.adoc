...
**Predicate:** `PersonTable` has primary key `PersonID`  a unique individual in the family tree with sex of `Sex`,
with a unique `UniqueID` (apparently a `GUID` or hash value that is a standard **GEDCOM** field), parent or parents
